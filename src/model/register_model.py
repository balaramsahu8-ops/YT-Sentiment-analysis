...
def register_model(model_name: str, model_info: dict):
    ...
    try:
        ...
        if not model_uri:
            model_uri = f"runs:/{model_info['run_id']}/{model_info.get('model_path', 'lgbm_model')}"
        ...
        if existing:
            latest = max(existing, key=lambda v: int(v.version))
            try:
                client.transition_model_version_stage(
                    name=model_name,
                ...
                logger.debug(f'Model {model_name} version {latest.version} already existed and was moved to Staging.')
                return
            except Exception:
                pass
